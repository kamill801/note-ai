...
This file governs Codex CLI/App work in `/Users/dd/Documents/note-ai`.

## Project

We are building a mobile-first YouTube knowledge capture app.
...
1. YouTube link registration.
2. Visible in-app YouTube player.
3. Current playback time capture.
4. Foreground capture mode.
5. App-internal voice trigger or manual fallback.
6. Voice memo recording and transcription.
7. Timestamp-based transcript segment selection.
8. Korean note generation with source evidence.
9. Follow-up search keywords/material recommendation.
10. Optional follow-up research job.
...
- Audio-only extraction from YouTube.
- Reading the official YouTube app's current playback state.
- System-wide always-listening custom wake word.
- Podcast support.
- Notion/Obsidian integration.
- Generic full-video summarizer as the main product.

## Technical Defaults

Assume this stack unless implementation evidence strongly suggests otherwise:

- Mobile: React Native + Expo Dev Client.
- Player: visible embedded YouTube player via WebView/IFrame bridge.
- Backend: TypeScript API.
- Database: PostgreSQL.
- Vector search: pgvector or compatible vector store.
- Background jobs: queue-backed workers.
- AI: speech-to-text, embeddings, structured LLM note generation, research synthesis.

## Development Rules

- The user intends to use OMX for implementation work in this repository.
- Prefer OMX-friendly handoffs: include workflow state, files touched, commands run, and verification evidence.
...
- Do not claim exact playback time in YouTube app linked mode.
- In estimated mode, use "추정 구간" and confidence.
- Make microphone/capture state explicit in UI.
- Preserve the user's voice memo in generated notes.
- Ground notes in transcript evidence when available.

## Verification

Before claiming a milestone complete, run the smallest meaningful checks:

- Unit tests for URL parsing and segment selection.
- Schema validation for AI outputs.
- API smoke tests for source/capture/note flows.
- Mobile smoke test for player, capture mode, and note display.
- Manual policy check: no hidden/background YouTube playback behavior.
